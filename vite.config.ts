...
const PRODUCTION_URL = 'https://web.ansible.su';
...
export default defineConfig(({ mode }): UserConfig => {
  ...
  const defaultAppTitle = `Ansible${appEnv !== 'production' ? ' Beta' : ''}`;
  ...
  const telegramApiId = env.ANSIBLE_API_ID || env.TELEGRAM_API_ID || '';
  const telegramApiHash = env.ANSIBLE_API_HASH || env.TELEGRAM_API_HASH || '';
  ...
  const plugins: PluginOption[] = [
    ...
    emitVersionFilesPlugin(APP_VERSION, APP_BUILD),
    viteStaticCopy({
  ...
  if (appEnv !== 'test' && (!telegramApiId || !telegramApiHash)) {
    throw new Error('Missing required API credentials (ANSIBLE_API_ID / ANSIBLE_API_HASH)');
  }
  ...
  setViteEnv({
    ...
    TG_WEB_DOMAIN: env.WEB_DOMAIN || 'web.ansible.su',
  });
...
function emitVersionFilesPlugin(appVersion: string, appBuild: string): Plugin {
  return {
    name: 'ansible:emit-version-files',
    apply: 'build',
    applyToEnvironment: (environment) => environment.name === 'client',
    generateBundle() {
      // именно emitFile, а не writeFileSync в closeBundle: каталог вывода к тому
      // моменту ещё не создан, и сборка падала на ENOENT.
      this.emitFile({ type: 'asset', fileName: 'version.txt', source: `${appVersion}\n` });
      this.emitFile({ type: 'asset', fileName: 'build.txt', source: `${appBuild}\n` });
    },
  };
}
...
function buildCsp(appEnv: string) {
  return `
  default-src 'self';
  connect-src 'self' wss://*.ansible.su blob: http: https: ${appEnv === 'development' ? 'wss: ipc:' : ''};
