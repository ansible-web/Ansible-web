...
addActionHandler('openChat', (global, actions, payload): ActionReturnType => {
  ...
  if (!chat) {
    ...
    const fetchChatPromise = isWithSelf
      ? callApi('fetchChat', { type: 'self', shouldRequestUpdate: true })
      : (user ? callApi('fetchChat', { type: 'user', user, shouldRequestUpdate: true }) : undefined);
    ...
    if (fetchChatPromise) {
      void fetchChatPromise.then((result) => {
        ...
        if (selectCurrentMessageList(globalNow, tabId)?.chatId !== id) return;
...
addActionHandler('openTelegramLink', async (global, actions, payload): Promise<void> => {
  ...
  const hostname = TME_WEB_DOMAINS.has(uri.hostname) ? 'asme.su' : uri.hostname;
